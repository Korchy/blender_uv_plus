            ...
            for i1, i in enumerate(polygon.loop_indices):
                ...
                if mesh_uv_loop.select:
                    ...
                    if edge_uv:
                        ...
                        # data['multi_points'][mesh_uv_loop.uv[:]]['edges_map'].append(EdgeUV(i1, Vector((edge_uv.vertices[0].x, edge_uv.vertices[0].y)), Vector((edge_uv.vertices[1].x, edge_uv.vertices[1].y))))
                        # data['multi_points'][mesh_uv_loop_next.uv[:]]['edges_map'].append(EdgeUV(i1, Vector2d(edge_uv.vertices[0].x, edge_uv.vertices[0].y), Vector2d(edge_uv.vertices[1].x, edge_uv.vertices[1].y)))
                        data['multi_points'][mesh_uv_loop.uv[:]]['edges_map'].append(
                            EdgeUV(i1, VertexUV(mesh_uv_loop, polygon_uv), VertexUV(mesh_uv_loop_next, polygon_uv))
                        )
                        data['multi_points'][mesh_uv_loop_next.uv[:]]['edges_map'].append(
                            EdgeUV(i1, VertexUV(mesh_uv_loop, polygon_uv), VertexUV(mesh_uv_loop_next, polygon_uv))
                        )
